...
void BaseDRAMSystem::PrintStats(unsigned int flips, unsigned int trr, unsigned int ecc_correctable,
    ...
    json_out.close();
    for (size_t i = 0; i < ctrls_.size(); i++) {
        ctrls_[i]->PrintFinalStats(flips, trr, ecc_correctable, ecc_detectable_uncorrectable, ecc_undetectable);
        if (i != ctrls_.size() - 1) {
